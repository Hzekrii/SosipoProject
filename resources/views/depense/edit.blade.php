        ...
        <div class="row justify-content-center">
            <div class="col-md-8">
                <div class="card  text-dark" style="background-color: ">
                    ...
                    <div class="card-body">
                        <form method="POST" class="needs-validation" novalidate="" action="{{ route('post.depense.edit',['id' => $depense->id]) }}" enctype="multipart/form-data" autocomplete="off">
                            @csrf
                            @method('PUT')
                            <div class="mb-3">
                                <label class="form-label" for="designation">Designation</label>
                                <input id="designation" type="text" class="form-control " name="designation" value="{{ old('designation',$depense->designation) }}" required autofocus>
                                @error('designation')
                            ...
                            <div class="mb-3">
                                ...
                                <input id="montant" type="text" class="form-control "
                                    ...
                                    readonly
                                    @endif
                                    name="montant" value="{{ $depense->montant }}" >
                                @error('montant')
                                <div class="invalid-feedback">{{ $message }}</div>
                                @enderror
                            </div>
                            <div class="mb-3">
                                ...
                                <select name="rubrique" class="form-select" id="rubrique">
                                    ...
                                    <option value="{{ $rubrique->id }}"
                                        @if ($depense->rubrique_id == $rubrique->id)
                                        selected
                                        @endif
                                        >{{ $rubrique->libelle }}</option>
                                    @endforeach
                                </select>
                                @error('rubrique')
                                <div class="invalid-feedback">{{ $message }}</div>
                            ...
                            <div class="mb-3">
                                <label class="form-label" for="feuille">Feuille de depense</label>
                                    <div>
                                        <a href="{{ url('depense/pdf/'.$depense->feuille) }}" class="btn btn-primary mb-3" target="_blank"><i class="bi bi-file-earmark-pdf"></i></a>
                                    </div>
