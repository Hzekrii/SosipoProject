                    ...
                    <ul class="navbar-nav">
                        ...
                        @if( auth()->user()->role_id != '3')
                            ...
                            <li class="nav-item mt-3">
                                ...
                                <li class="nav-item mt-3">
                                    <a class="nav-link  btn btn-gradient-success text-light  "  href="{{ route('rembourssement.show') }}">
                                        <div class="icon icon-shape icon-sm border-radius-md text-center me-2 d-flex align-items-center justify-content-center">
                                        <i class="bi bi-graph-down-arrow text-light text-lg "></i>
                                        </div>
                        ...
                        @if(Auth::user()->role_id =="1")
                            <li class="nav-item dropdown text-dark pe-2 mx-3 d-flex align-items-center">
                                ...
                                <ul class="dropdown-menu  dropdown-menu-end text-dark  px-2 py-3 me-sm-n4" aria-labelledby="dropdownMenuButton">
                                    ...
                                    <li class="mb-2">
                                        <a class="dropdown-item text-dark border-radius-md" href="{{ route('approuve.credit.show') }}">
                                            <div class="d-flex py-1">
                                            <div class="d-flex flex-column justify-content-center">
                                                <div class="d-flex justify-content-between">
                                                    <div class="col me-5">
                                                    ...
                                                    <div class="col">
                                                        <p class="badge text-danger me-auto">{{ DB::table('credits')->where('approuve', false)->count() }}</p>
                                                    </div>
                                                </div>
                                                <p class="text-xs text-secondary mb-0">
                                                <i class="fa fa-clock me-1"></i>
                                                    @php
                                                                $credit = DB::table('credits')->where('approuve',false)->first();
                                                                if($credit)
                                                                {
                                                                    $created_at = Carbon::createFromFormat('Y-m-d H:i:s', $credit->created_at);
                                                                    $diff = $created_at->diffForHumans();
                                                                    echo $diff;
                                                                }else   echo '<p class="text-xs text-secondary mb-0">Tout les credits sont approuvé.</p>'
                                                    @endphp
                                                </p>
                                            </div>
                                            </div>
                                    ...
                                    <li class="mb-2">
                                        <a class="dropdown-item text-dark border-radius-md" href="{{ route('approuve.rembourssement.show') }}">
                                            <div class="d-flex py-1">
                                            <div class="d-flex flex-column justify-content-center">
                                                <div class="d-flex justify-content-between">
                                                    <div class="col me-5">
                                                        <p class="text-sm font-weight-normal mb-1">Remboursements</p>
                                                    </div>
                                                    <div class="col">
                                                        <p class="badge text-danger me-auto">{{ DB::table('rembourssements')->where('approuve', false)->count() }}</p>
                                                    </div>
                                                </div>
                                                <p class="text-xs text-secondary mb-0">
                                                <i class="fa fa-clock me-1"></i>
                                                    @php
                                                                $rembourssement = DB::table('rembourssements')->where('approuve',false)->first();
                                                                if($rembourssement)
                                                                {
                                                                    $created_at = Carbon::createFromFormat('Y-m-d H:i:s', $rembourssement->created_at);
                                                                    $diff = $created_at->diffForHumans();
                                                                    echo $diff;
                                                                }else   echo '<p class="text-xs text-secondary mb-0">Tout les rembourssements sont approuvé.</p>'
                                                    @endphp
                                                </p>
                                            </div>
                                            </div>
