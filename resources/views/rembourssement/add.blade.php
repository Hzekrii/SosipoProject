        ...
        <div class="col">
            <div class="card text-dark">
                <div class="card-body">
                    ...
                    <form method="POST" class="needs-validation" novalidate="" action="{{ route('rembourssement.add') }}" enctype="multipart/form-data" autocomplete="off">
                        @csrf
                        <div class="mb-3">
                            ...
                            <select name="credit_id" class="form-select" aria-label="Default select example">
                                ...
                                @foreach ($credits as $credit)
                                    @if ($credit->approuve==1)
                                        <option value="{{ $credit->id }}" @if (old('credit_id') == $credit->id) selected @endif>
                                            {{ $credit->designation }}
                                        </option>
                                    @endif
